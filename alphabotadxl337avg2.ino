...
float SETAccY;
float SETAccZ;
volatile float scaledX, scaledY, scaledZ;
volatile float avgx, avgy, avgz;
...
void ISR_timerone()
{
 ...
 scaledX = mapf(rawX, 0, 1023, -scale, scale); // 3.3/5 * 1023 =~ 675
  scaledY = mapf(rawY, 0, 1023, -scale, scale);
 // scaledZ = mapf(rawZ, 0, 1023, -scale, scale);
  ACCX=(9.81*(scaledX-SETAccX)*5);
ACCY=(9.81*(scaledY-SETAccY)*5);
// ACCZ=(9.81*(scaledZ)*0.5);
...
Serial.println(avgy);
 // Serial.print(",");
//  Serial.print(avgz); Serial.println(" g");
...
void AVG(){ 
  ...
  while (millis()-curr<999)//Χρόνος εξομάλυνσης 0,999s
  {
    count++;//Αύξησε πλήθος μετρήσεων
    x=ACCX;//Διάβασε επιτάχυνση x
    y=ACCY;//Διάβασε επιτάχυνση y 
   // z=ACCZ;//Διάβασε επιτάχυνση z
