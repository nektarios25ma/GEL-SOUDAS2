...
unsigned char Lpwm_val = 151;
unsigned char Rpwm_val = 155;
int Car_state=0;
int divtime;
 ...
 FrontDetection();
     ...
     //    Serial.print(" +5 velocity right:  "); Serial.print(Lpwm_val);  Serial.println(Rpwm_val);
       ...
       // Serial.print(" -5 velocity : "); Serial.print(Lpwm_val);  Serial.println(Rpwm_val); 
        lcd.clear();
        lcd.setCursor(0, 1);
        lcd.print("-5velocity"); lcd.print(Lpwm_val);  lcd.print(Rpwm_val);
        }
        break;
       ...
       FrontDetection();
        start_time=millis();
while ((millis() - start_time < 20000UL) && (FrontDistance > 4 )) { // Εφόσον δεν πέρασαν 200 δευτ και η απόσταση απο εμπόδιο είναι μεγαλύτερη απο 4cm κάνε:
    ...
    else
    {
     ...
     stopp();   
        ...
        for(int i=34; i<170;i++ ){
               
               Set_Speed(i,i+6);
              advance();
              lcd.clear();
              lcd.setCursor(0, 1);
        lcd.print("+accel   "); lcd.print(i); lcd.print(i+6);
       ...
       FrontDetection();
        ...
        for(int j=150; j>=30; j-- ){
              ...
              lcd.clear();
              lcd.setCursor(0, 1);
        lcd.print("-accel   "); lcd.print(j); lcd.print(j+4);
        delay(25);
        FrontDetection();
        if (j < 35)  
      {
