...
void ISR_timerone()
{
  ...
  volatile int x = analogRead(xpin);
  
 volatile int y = analogRead(ypin);
  
 volatile int z = analogRead(zpin);
...
Serial.print(",");///
 Serial.print(speed2);
 Serial.print(",");
 Serial.print(((float)x - zero_G_x)/scale_x*9.8);
  Serial.print(",");

  Serial.println(((float)y - zero_G_y)/scale_y*9.8);
  //Serial.print(",");

  //Serial.print(((float)z - zero_G_z)/scale_z*9.8);
